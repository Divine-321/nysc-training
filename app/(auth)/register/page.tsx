  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    if (awaitingOtp) {
      ...
      try {
        ...
        router.push(role === "admin" || role === "superadmin" ? "/admin/dashboard" : "/staff/dashboard");
    ...
    if (!formData.surname || !formData.email || !formData.password || !formData.fileNo || !formData.phone) {
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-8 sm:p-12 overflow-y-auto">
        ...
        {success && (
          <div className="fixed top-6 right-6 z-50 bg-white border-l-4 border-green-500 p-4 rounded-xl shadow-2xl flex items-center gap-3 w-80 transform transition-all duration-300">
            ...
            <p className="text-sm text-gray-700 font-medium flex-1">
              {awaitingOtp ? "Registration successful. Check your email for the OTP." : "Registration successful."}
            </p>
          </div>
        )}
        ...
        <form onSubmit={handleRegister} className="w-full max-w-sm space-y-4">
          ...
          <div>
            ...
            <input type="tel" placeholder="e.g. 08012345678" value={formData.phone} onChange={(e) => setFormData({ ...formData, phone: e.target.value })} className="w-full border border-[#1a6b3c] rounded-full px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-[#1a6b3c]" />
          </div>
          <div>
            ...
            <div className="relative">
              ...
              <button type="button" aria-label={showPassword ? "Hide password" : "Show password"} onClick={() => setShowPassword(!showPassword)} className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-500 hover:text-[#1a6b3c] transition">
          ...
          <button type="submit" disabled={isLoading} className="flex items-center justify-center w-full bg-[#1a6b3c] hover:bg-[#145530] text-white font-semibold py-3 rounded-full transition disabled:opacity-70 disabled:cursor-not-allowed mt-2">
            {isLoading ? "Please wait..." : awaitingOtp ? "Verify Email" : "Create Account"}
          </button>
        </form>
      </div>
    </div>
